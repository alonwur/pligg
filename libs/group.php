...
function group_stories($requestID,$catId)
{
	...
	$links = $db->get_results($sql = "SELECT SQL_CALC_FOUND_ROWS * FROM " . table_links . " WHERE link_group_id = $requestID AND link_group_status!='discard' $from_where ORDER BY link_published_date DESC, link_date DESC LIMIT $offset, $page_size");
